...
from validators import (
    WorkerCreateSchema, WorkerUpdateSchema, AdvanceCreateSchema,
    AttendanceSchema, SalaryGenerateSchema, MarshmallowValidationError
)
...
def create_worker():
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid worker data: {e.messages}",
...
def update_worker(worker_id):
    ...
    try:
        ...
        raise ValidationError(
            ...
            code="WORKER_UPDATE_VALIDATION_FAILED"
        )
...
def add_advance(worker_id):
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid advance data: {e.messages}",
            code="ADVANCE_VALIDATION_FAILED"
        )
...
def generate_salary(worker_id):
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid salary data: {e.messages}",
            code="SALARY_VALIDATION_FAILED"
        )
...
def mark_attendance(worker_id):
    ...
    check_in = datetime.strptime(validated['check_in'], '%H:%M').time() if validated.get('check_in') else None
    check_out = datetime.strptime(validated['check_out'], '%H:%M').time() if validated.get('check_out') else None
