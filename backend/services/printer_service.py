    ...
    def build_esc_pos_bytes(self) -> bytes:
        ...
        for cmd_type, val in self.commands:
            if cmd_type == "text":
                ...
                # ESC E 1 = bold ON only (no double-strike) → ~1.5× bold effect
                ...
                stream.extend(b"\x1d!\x11")
            elif cmd_type == "double_height_off":
                # Restore to 1.5x global size (1x width × 2x height)
...
class PrinterService:
    ...
    def _build_bill_receipt(self, bill_data: Dict, settings: Dict) -> ReceiptBuilder:
        ...
        # Cashier & Bill No on same line (image: "Cashier: <NAME>  Bill No.: <NUM>")
        cashier = (
