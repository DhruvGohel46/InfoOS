...
def build_bill(order: Dict, settings: Dict) -> bytes:
    ...
    subtotal = float(order.get("subtotal") or order.get("sub_total") or calculated_subtotal)
    ...
    # Grand Total
    grand_total = float(
        order.get("grandTotal") or order.get("grand_total") or order.get("total") or subtotal
    )
    formatter.divider()
    formatter.bold_on()
...
def build_test_print(settings: Dict) -> bytes:
    ...
    formatter.set_text_size(TextSize.DOUBLE_WIDTH).line("Double Width").set_text_size(
        TextSize.NORMAL
    )
    formatter.set_text_size(TextSize.DOUBLE_HEIGHT).line("Double Height").set_text_size(
        TextSize.NORMAL
    )
